...
> **历史研究，当前方案已撤回。** 这里的 `Sxx` 确实是 Artkal S-5mm 官方色号，但用户随后确认需要的是 MARD 221 的 `A1 / C19 / E11` 编号体系。当前标准见 [`mard-221-numbering-standard.md`](mard-221-numbering-standard.md)。

## 结论

本文件记录曾评估过的 **Artkal S-5mm Midi 硬豆** 方案。图纸中的 `S01`、`S13`、`S54` 等均为 Artkal 官方 S 系列色号，不是项目自编号；但该体系不符合用户最终确认的编号模式，不再作为第 010 集实做依据。

Hama 官方色卡使用纯数字编号；Perler 主要使用颜色名与商品货号。当时只根据“字母＋数字”和公开色卡条件，曾误把 Artkal S-5mm 选为本集方案；用户给出的范例随后证明目标实际是 MARD 221 短码。
